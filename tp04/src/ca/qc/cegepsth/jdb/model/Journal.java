...
public class Journal {
	...
	private ArrayList<Calendar> SetTimeOrderCorrectly(Calendar bTime,
		...
		if (EndTime.getTimeInMillis() > BeginTime.getTimeInMillis()) {
			// Si on se rend ici, le temps de fin est avant le début et on doit
			// les Swapper
			Calendar Temp;
			Temp = BeginTime;
			BeginTime = EndTime;
	 ...
	 * 
	 *         Retourne le dernier évènement dans la liste selon le type entré
	 *         en paramètre
	 */
	...
	public boolean saveToDevice(Context context) {
		...
		try {
			...
			outputStream.write(aSauvegarder.getBytes()); // on écrit le contenu
															// de la string
			outputStream.close(); // fermeture du Fichier
		} catch (IOException e) {
			// Normalement on ne devrait pas arriver ici, SI c'est le cas on
			// affichera un messageBox explicant le problème
		...
		return true; // On retourne Vrai si il n'y a aucun problème et que on se
						// rend jusqu'ici
	}
	...
	public List<EvenementJournal> loadFromDevice(Context ctx) {
		...
		try {
			...
			if (!serialisedObject.exists()) {
				...
				// Création des instances a  partir du fichier, groupe en une
				// collection
				Gson gson = new Gson();
			...
			// Normalement on ne devrait pas arriver ici, SI c'est le cas on
			// affichera un messageBox explicant le problème
			AlertDialog.Builder messBoxAlert = new AlertDialog.Builder(ctx);// crée
																			// un
																			// alert
																			// messageBox
